class MocaFormController < ApplicationController
  before_action :set_subject_and_date, only: [:create]

  def index
    @moca_form_rows = MocaFormRow.all
    render json: @moca_form_rows, status: :ok
  end

  def create
    row = MocaFormRow.new(moca_row_params(params))
    row.subject = @subject
    row.date = @date
  ...
  def download_data
    @results = MocaFormRow.where(assessment_id: params[:id])
  ...
  def mark_assessment
    if !@subject.nil?
      @assessment = Assessment.find_or_initialize_by(:subject => @subject, :date => @date, user: @current_user, moca: false)
      @assessment.update_attributes!(moca: true)
    end
  end

  def moca_row_params(row_params)
